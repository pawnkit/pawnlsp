...
func TestServerReturnsHTTPAPICompletion(t *testing.T) {
	uri := tempDocumentURI(t)
	text := "main() { HT }"
	var input bytes.Buffer
	frame(t, &input, map[string]any{"jsonrpc": "2.0", "id": 1, "method": "initialize", "params": map[string]any{}})
	...
	frame(t, &input, map[string]any{"jsonrpc": "2.0", "id": 2, "method": "textDocument/completion", "params": map[string]any{
		"textDocument": map[string]any{"uri": uri}, "position": map[string]any{"line": 0, "character": 11},
	}})
	frame(t, &input, map[string]any{"jsonrpc": "2.0", "method": "exit"})
	...
	for _, value := range []string{`"label":"HTTP"`, "HTTP_METHOD:method", "const callback[]"} {
		if !strings.Contains(output.String(), value) {
			t.Fatalf("HTTP completion missing %q: %s", value, output.String())
		}
	}
}
